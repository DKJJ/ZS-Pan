...
# crism dataset
class crism_dataset(data.Dataset):
    def __init__(
            self, config, is_train=True, is_dhp=False, want_DHP_MS_HR=False
    ):
        self.split = "train" if is_train else "val"  # Define train and validation splits
        ...
        self.want_DHP_MS_HR = want_DHP_MS_HR  # This ask: DO we need DIP up-sampled output as dataloader output?
        self.is_dhp = is_dhp  # This checks: "Is this DIP training?"
        self.dir = self.config["crism_dataset"]["data_dir"]  # Path to crism dataset
        ...
        self.augmentation = self.config["crism_dataset"]["augmentation"]  # Augmentation needed or not?

        self.LR_crop_size = (
        self.config["crism_dataset"]["LR_size"], self.config["crism_dataset"]["LR_size"])  # Size of the the LR-HSI

        self.HR_crop_size = [self.config["crism_dataset"]["HR_size"],
                             self.config["crism_dataset"]["HR_size"]]  # Size of the HR-HSI
